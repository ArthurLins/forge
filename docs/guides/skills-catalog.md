...
| **forge-validate**        | command | Statically validate the project's structural integrity — prompt state machine, requirement tags, the Conventions Map, the config, and derived-doc freshness. Read-only; PASSES trivially on an empty project. | `/forge-validate`          |
